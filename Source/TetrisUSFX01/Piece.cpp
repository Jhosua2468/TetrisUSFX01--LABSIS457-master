...
void APiece::SpawnBlocks()
{
    for (int i = 0; i < Blocks.Num(); i++)
    {
        ...
        SpawnLocation.SetLocation(Blocks[i]->GetActorLocation() + GetActorLocation());

        // Spawn a new block and set its location
        ABlock* SpawnedBlock = GetWorld()->SpawnActor<ABlock>(ABlock::StaticClass(), SpawnLocation);
        SpawnedBlock->SetActorLocation(SpawnLocation.GetLocation());
    }
